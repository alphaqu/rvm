...
impl<'ctx> Executor<'ctx> {
	...
	pub fn prepare(&self, bindings: &Bindings, reference: &Reference) {
		...
		match reference {
			Reference::Method(method) => {
				...
				if self.exec.get_function_address(&fn_name).is_err() {
					self.compile_relay(bindings, method);
				}
			}
		}
	}

	fn compile_relay(&self, bindings: &Bindings, reference: &MethodReference) {
				...
				&[
					builder
						.build_int_to_ptr(
							self.ctx
								.i64_type()
								.const_int(bindings.runtime_global as *const _ as u64, false),
	...
	pub fn compile_method(&self, bindings: &Bindings, method: &Method, cp: &ConstantPool) -> usize {
		if self
			.initialized
			...
			.is_ok()
		{
			...
			self.exec
				.add_global_mapping(&value, bindings.resolve_method as usize);
		}
		...
		let mut data = self.compute_blocks(&method.code.instructions);
		...
		self.resolve_blocks(bindings, &mut data, cp);
	...
	fn compile_blocks(&self, method: &Method, data: BlocksData<'_, 'ctx>) {
		...
		if let Some(caller) = self.module.get_function(&method.call_name) {
	...
	fn resolve_blocks(&self, bindings: &Bindings, data: &mut BlocksData, cp: &ConstantPool) {
		...
		for reference in references {
			self.prepare(bindings, &reference);
		}
	}
			...
			if let Some(last) = last {
				match last {
                    Inst::Jump(inst) => {
						...
						let target_inst = (inst.offset as isize + this.get_end_idx() as isize) as usize;
						let target = *inst_to_block.get(&target_inst).expect("Could not find target");
