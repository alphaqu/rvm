...
impl ArrayClass {
    ...
    pub fn size(&self, obj: Ref) -> usize {
	    // reuse that method
	    let length = Array::<()> { reference: obj, _p: Default::default() }.get_length();
	    ARRAY_BASE_OFFSET + (self.component.size() * (length as usize))
    }
...
//
	...
	fn get_array_class(&self, class_id: Id<Class>) -> JResult<MappedRwLockReadGuard<RawRwLock, ArrayClass>> {
		let guard = self.cl.get(class_id);
		match &guard.kind {
			ClassKind::Array(_) => {
				Ok(MappedRwLockReadGuard::map(guard, |v| {
			...
			_ => {
				return Err(JError::new("Expected array class"))
			}
		}
...
impl<T: Type> Array<T> {
	...
	unsafe fn get_index_ptr(&self, idx: i32) -> *mut u8 {
		self.reference.ptr().add(ARRAY_BASE_OFFSET + (T::ty().size() * (idx as usize)))
	}
