use crate::{Class, ClassInfo, ClassKind, JResult, MethodIdentifier, NativeCode, StrParse};
...
impl ClassLoader {
    ...
    pub fn get(&self, id: Id<Class>) -> MappedRwLockReadGuard<'_, RawRwLock, Class> {
        RwLockReadGuard::map(self.classes.read(), |v| {
            v.get(id)
        })
    }
    ...
    #[deprecated]
    pub fn get_obj_class(&self, id: Id<Class>) -> MappedRwLockReadGuard<'_, RawRwLock, ObjectClass> {
        MappedRwLockReadGuard::map(self.get(id), |v| {
            match &v.kind {
    ...
    pub fn scope_class<R>(&self, id: Id<Class>, mut func: impl FnOnce(&ObjectClass) -> R) -> R {
    ...
    pub fn load_jar(&mut self, data: Vec<u8>, filter: impl Fn(&str) -> bool) -> anyways::Result<()> {
        ...
        for name in map {
            ...
            if file.is_file() && file.name().ends_with(".class") {
                if filter(file.name()) {
                    ...
                    self.load_class(data).wrap_err_with(|| format!("Failed to load {}", file.name()))?;
                }
            }
        }
        Ok(())
