...
impl Method {
    pub fn new(name: String, desc: String, flags: MethodAccessFlags, code: MethodCode) -> (MethodIdentifier, Method) {
        (
            MethodIdentifier {
                name: name.to_string(),
    ...
    pub fn parse(
        ...
        if info.access_flags.contains(MethodAccessFlags::NATIVE) {
            match class_loader.native_methods().get(&(class_name.to_string(), ident.clone())) {
                None => {
	                return Err(Audit::new(format!("Could not find native method {class_name} {ident:?}")));
                }
                Some(method) => {
