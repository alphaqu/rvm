        ...
        match bc.returns().clone() {
            ReturnDescriptor::Field(value) => {
                ...
                let value = match value.ty() {
                    ValueType::Boolean => {
                        bc.build_int_cast(value_enum.into_int_value(), bc.boolean(), "return").as_basic_value_enum()
                    }
                    ValueType::Byte => {
                        bc.build_int_cast(value_enum.into_int_value(), bc.i8(), "return").as_basic_value_enum()
                    }
                    ValueType::Short => {
                        bc.build_int_cast(value_enum.into_int_value(), bc.short(), "return").as_basic_value_enum()
                    ...
                    ValueType::Long => {
                        bc.build_int_cast(value_enum.into_int_value(), bc.long(), "return").as_basic_value_enum()
                    }
                    ValueType::Char => {
                        bc.build_int_cast(value_enum.into_int_value(), bc.char(), "return").as_basic_value_enum()
                    }
                    ValueType::Float => {
                        bc.build_float_cast(value_enum.into_float_value(), bc.float(), "return").as_basic_value_enum()
                    }
                    ValueType::Double => {
                        bc.build_float_cast(value_enum.into_float_value(), bc.double(), "return").as_basic_value_enum()
                    }
                    ValueType::Reference => {
                        value_enum
                    }
...
impl Display for ReturnTask  {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "return {}", if let Some(value) = &self.returns {
            format!("{value:?}")
        } else {
            format!("")
        })?;
