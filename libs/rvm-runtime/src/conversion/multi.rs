...
pub trait FromJavaMulti: Sized {
	fn from_vec(vec: Vec<AnyValue>, runtime: &Arc<Runtime>) -> eyre::Result<Self>;
}
pub trait ToJavaMulti: Sized {
	fn to_vec(self, runtime: &Arc<Runtime>) -> eyre::Result<Vec<AnyValue>>;
}
...
macro_rules! impl_from_java_multi {
    ($($V:ident),*) => {
		...
		impl<$($V: FromJava),*> FromJavaMulti for ($($V),*) {
			fn from_vec(mut vec: Vec<AnyValue>, runtime: &Arc<Runtime>) -> eyre::Result<Self> {
